...
#     return False
def get_gps_time():
    # Simulate GPS time with system time
...
#     return None
def passive_gps_time():
    # Simulate passive GPS time with current time in seconds since midnight
...
def get_gps_data():
    # Simulate GPS data
    gps_data = {'lat': 40.7128, 'lon': -74.0060, 'pdop': 1.0}
    return gps_data
...
# Specify the DataLogs directory within the current script's directory
log_file_dir = os.path.join(current_script_dir, "DataLogs")

# Ensure the DataLogs directory exists, create if it doesn't
